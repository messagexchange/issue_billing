class SupportReport < Prawn::Document
  ...
  def to_pdf(issues, project, total_hours, start_date, end_date)
    ...
    text "#{start_date} to #{end_date} (inclusive)"
  ...
  def create_issues_table(issues)
    ...
    issues.each do |i|
      table << [i.id.to_s, i.subject, format_time(i.created_on), i.raised_by, (i.assigned_to.nil?) ? i.author.to_s : i.assigned_to.to_s, get_billable_hours(i.hours).to_s]
    end
